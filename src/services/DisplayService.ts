...
import {
  success, highlight, warning, error,
} from '../utilities/loggingFormat.js';
...
export class DisplayService implements IDisplayService {
  ...
  displayPackage(toDisplay: Package): void {
    const message = `${success(`Package summary for order ${toDisplay.orderId}`)}\r\n${
      this.separator
  ...
  private invoiceDisplay(toDisplay: Package): string {
    ...
    for (const [article, count] of toDisplay.invoice?.articles ?? []) {
  ...
  private toolsDisplay(toDisplay: Package): string {
    ...
    for (const tool of toDisplay.tools ?? []) {
  ...
  private restockingDisplay(toDisplay: Package): string {
    ...
    for (const [article, count] of toDisplay.restocking?.articles ?? []) {
